...
knitr::kable(colors_summary_df, caption = "summary of all colors Distribution")
```
...
red_1<- MnM_sample %>%
  ...
  labs(title="Distribution of Eveness of Colors",
       x= "count of evens",
       y= "count",
       fill= "is even"
       )
...
red_2<- MnM_sample %>%
  ...
  geom_bar()+
  labs(title="Distribution of the Red Color",
       x= "No. of red",
       y= "count",
       fill= "is even")+
...
MnM_sample %>% #plot density
  ...
  labs(title = "Color Variance vs Gamma Distribution",
       subtitle = "comparing Variance of lentils colors to equivalent Gamma distribution",
       x = "Variance",
       y = "") +
...
MnM_sample %>%
  dplyr::select(1:6) %>%
  mutate(count_even = rowSums(across(everything(), ~ .x %% 2 == 0))) %>%
  count(count_even) %>%
...
The simulation created a random samples of snack packs, which was proven to be statistically random with known $\mu$ and $\sigma^2$. I created one sample with specific size and numbers of colors using "sample_MnM", and costume multiple samples using "mega_snack". Then, I check the relevant indicators for this project.
...
2. alternatively, I would recommend calculate the amount of each type in small packages
...
[the M&M Pack Simulation Game](https://yonigr94.github.io/Simulation-application-of-M-M//)
