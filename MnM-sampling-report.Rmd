...
create_bag- function to create one snack package as matrix.
...
MnM_sample %>%
  ...
  geom_bar()+labs(title="Example of One Color Distibution")
```
...
round(as.data.frame(p_value_matrix),4) %>%
  ...
  pivot_longer(cols = V1:V6, names_to = "color_2", values_to = "t_test") %>% 
...
mega_snack<- function(nn,n_unit,n_color)
  ...
  for (i in 1:(dim(res)[1]))
  {
    ...
    small_sample<-small_sample %>% as_data_frame() %>% 
...
mega_snack_1 %>%
  ...
  labs(title = "Heatmap of avarage even_count by number of colors and units",
       x = "Lentils",
       y = "No. of colors",
       fill = "even_count") +
  theme_minimal()
...
mega_snack_1 %>%
  ggplot(aes(x = as.factor(round( n_unit,1) ), y = n_color , color = all_even,size = 8 )) +
  geom_point() +
  ...
  labs(title = "Heatmap of mega_snack all evens",
       ...
       y = "No. of colors",
       color = "all_even") +
        geom_text(aes(label = all_even * 100), color = "black", size = 4)+
  theme_minimal()
```
