...
```{r sample one bag, error=FALSE, warning=FALSE}
...
ggplot the MnM_sample sample:
```{r ggplot 1 sample, warning=FALSE, error=FALSE}
#check how many evens there are
...
# יצירת מטריצה ריקה לשמירת התוצאות
...
we will create a function that create sample for each number of colors and package size we want,
and then calculate some interesting parameters
...
color: 2:7
pieces per package: 9.1,45,100,200,500,1000
